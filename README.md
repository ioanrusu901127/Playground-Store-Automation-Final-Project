...
test.describe('Inventory Management', () => {
  ...
  test('should add new product to inventory', async ({ page }) => {
    ...
    // Act
    await inventoryPage.addProduct(productName, price, quantity);

    // Assert
    await expect(inventoryPage.getProductByName(productName)).toBeVisible();
    expect(await inventoryPage.getProductPrice(productName)).toBe(price);
    expect(await inventoryPage.getProductQuantity(productName)).toBe(quantity);
  });
});
```
...
test('complete purchase flow', async ({ page }) => {
  await test.step('Add product to inventory', async () => {
    await inventoryPage.addProduct('Laptop', 999, 5);
  });

  await test.step('Add product to cart from catalog', async () => {
    await catalogPage.goto();
    await catalogPage.addToCart('Laptop');
  });
  ...
  await test.step('Verify order created', async () => {
    ...
    await expect(ordersPage.latestOrder).toContainText('Laptop');
  });
});
```
